...
namespace Unluau.Disassembler
{
    ...
    public class Lifter : BinaryReader
    {
        ...
        /// </summary>
        /// <param name="fileInfo">The file to lift.</param>
        public Lifter(FileInfo fileInfo) : base(fileInfo.OpenRead()) => _source = fileInfo.Name;

        /// <summary>
        /// Creates a new instance of <see cref="Lifter"/>.
        /// </summary>
        /// <param name="input">The input stream.</param>
        public Lifter(Stream input) : base(input)
        {
        }
        ...
        public Lifter(Stream input, Encoding encoding) : base(input, encoding)
        {
        }
        ...
        public Lifter(Stream input, Encoding encoding, bool leaveOpen) : base(input, encoding, leaveOpen)
        {
        }
        ...
        public ProtoType LiftProtoType()
        {
            ...
            if (ReadBoolean())
            {
                ...
                {
                    instruction.LineDefined = absLineInfo[pc >> lineGapLog2] + lineInfo[pc];

                    // If the instruction has an auxiliary instruction, then we need to skip the next instruction.
                    if (instruction.Aux != null)
                    {
                        pc++;
                        instruction.Aux.LineDefined = absLineInfo[pc >> lineGapLog2] + lineInfo[pc];
                    }
